# Sprint Pointing App - HTML Template (V3)

Complete HTML template for the Sprint Pointing App widget with V3 enhancements.

## V3 Enhancements in This File

- Duck emoji (🐥) for Pass card
- Timer visible to all participants (moved to header)
- Participant avatars with initials fallback
- Story fields reordered (Sprint before Points)
- Reference fields for Assignment Group, Assigned To, Sprint
...
4. Replace the entire HTML with the code below
5. Click **Save**
...
<div class="sprint-pointing-app">
  ...
  <header class="app-header">
    ...
    <div class="header-right">
      ...
      <!-- V3: Timer Display (visible to all participants) -->
      <div class="timer-display" ng-if="c.state.ui.timerActive">
        <i class="fa fa-clock-o"></i>
        <span class="timer-text">{{c.getTimerDisplay()}}</span>
      </div>
      
      <!-- V3: Participant Count with Avatars -->
  ...
  <main class="main-content" ng-if="c.state.session.id">
    ...
    <section class="center-panel">
      <div ng-if="c.getCurrentStory()" class="story-detail">
        ...
        <!-- V3: Story Metadata Grid (Reordered: Sprint before Points) -->
        <div class="story-metadata-grid">
          ...
          <div class="metadata-item">
            ...
            <input ng-if="c.editMode.sprint" 
                   type="text" 
                   class="form-control form-control-sm"
                   ng-model="c.editValues.sprint"
                   ng-blur="c.saveField('sprint')">
          </div>
    ...
    <aside class="right-panel">
      ...
      <div class="voting-panel-content" ng-switch="c.getVotingPanelState().view">
        ...
        <div ng-switch-when="SHOW_CARDS" class="voting-cards-section">
          ...
          <!-- V3: Voting Cards with Duck Emoji for Pass -->
          <div class="cards-grid">
            <div ng-repeat="card in c.cardValues track by $index" 
              ...
              <span class="card-value" ng-if="card !== 'pass'">{{card}}</span>
              <span class="card-value card-pass" ng-if="card === 'pass'">🐥</span>
            </div>
          </div>
        ...
        <div ng-switch-when="VOTE_SUBMITTED" class="vote-submitted-section">
          <div class="submitted-confirmation">
            ...
            <div class="your-vote">
              Your vote: <strong class="vote-value">
                <span ng-if="c.state.voting.selectedCard !== 'pass'">{{c.state.voting.selectedCard}}</span>
                <span ng-if="c.state.voting.selectedCard === 'pass'">🐥 Pass</span>
              </strong>
            </div>
          </div>
          ...
          <!-- V3: Participant Vote Status with Avatars -->
          <div class="participant-status" ng-if="c.state.participants">
            <div ng-repeat="(id, participant) in c.state.participants" class="participant-item">
              <div class="participant-avatar" ng-if="participant.photo">
                <img ng-src="{{participant.photo}}" alt="{{participant.name}}">
              </div>
              <div class="participant-avatar participant-initials" ng-if="!participant.photo">
                {{c.getInitials(participant.name)}}
              </div>
        ...
        <!-- V3: Show Results (with vote results visible to all) -->
        <div ng-switch-when="SHOW_RESULTS" class="results-section">
          ...
          <div class="distribution-chart">
            ...
            <div ng-repeat="(value, count) in c.state.voting.results.distribution track by value" 
              ...
              <span class="dist-label">
                <span ng-if="value !== 'pass'">{{value}}</span>
                <span ng-if="value === 'pass'">🐥</span>
              </span>
          ...
          <!-- V3: Individual Votes (visible to all participants) -->
          <div class="individual-votes">
            ...
            <div ng-repeat="vote in c.state.voting.results.votes track by vote.voter_name" 
              ...
              <div class="voter-info">
                <div class="participant-avatar" ng-if="vote.voter_photo">
                  <img ng-src="{{vote.voter_photo}}" alt="{{vote.voter_name}}">
                </div>
                <div class="participant-avatar participant-initials" ng-if="!vote.voter_photo">
                  {{c.getInitials(vote.voter_name)}}
                </div>
                <span class="voter">{{vote.voter_name}}</span>
              </div>
              <span class="estimate">
                <span ng-if="vote.estimate !== 'pass'">{{vote.estimate}}</span>
                <span ng-if="vote.estimate === 'pass'">🐥</span>
              </span>
            </div>
          </div>
...
1. **Duck Emoji for Pass**: Pass card shows 🐥 instead of "pass" text
2. **Timer in Header**: Timer moved to header, visible to all participants
3. **Participant Avatars**: Avatar display with photo or initials fallback
4. **Story Fields Reordered**: Sprint field moved before Points in metadata grid
5. **Reference Fields**: Assignment Group, Assigned To, Sprint are editable
6. **Vote Results Visible**: All participants can see individual votes after reveal

## Testing

After updating the HTML:

1. Open a Sprint Pointing session
2. Verify the Pass card shows 🐥
3. Start a timer (moderator) → verify it appears in header for all users
4. Check participant avatars show initials or photos
5. Verify Sprint field appears before Points in story details
6. Test editing reference fields (moderator only)
7. After revealing votes, verify all participants can see individual votes

## Requirements Validated

- 3.1: Duck emoji for Pass card ✓
- 4.1: Timer visible to all participants ✓
- 6.3: Participant avatars with initials ✓
- 8.1: Sprint before Points ✓
- 8.2: Reference fields editable ✓
- 9.1: Vote results visible to all ✓
